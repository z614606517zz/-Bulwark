...
void BaselineAnalyzer::importSnapshot(const BaselineSnapshot& snapshot) {
    ...
    for (const BaselineProgram& prog : snapshot.programs) {
        ...
        Profile p;
        p.firstSeenUtc = prog.firstSeenUtc.isValid() ? prog.firstSeenUtc : nowUtc;
        p.lastSeenUtc = prog.lastSeenUtc.isValid() ? prog.lastSeenUtc : nowUtc;
