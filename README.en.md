...
- **Behavior prompt (PromptDialog)** — shown when no rule matches and the actor is untrusted. A risk-colored header + level badge; two cards (digital signature + VirusTotal intel); program / description / command line / behavior (with ATT&CK tags) / target details; SHA256 + risk score; expandable **Process lineage** and **Evidence timeline**; an **🤖 AI security assistant** that generates an attack narrative; and at the bottom "Remember my choice" + scope dropdown (Permanent / This session / 1 hour / 1 day) + **✓ Allow** / **✕ Block**.
- **Block notification (ToastWindow · Block)** — a corner toast when a deterministic high-risk action is blocked outright (stacked/managed by `ToastNotifier`).
- **AI scan toast (ToastWindow · AiScan)** — a lightweight toast when an AI verdict is triggered by launching a program.
