...
### 新增 ✨
- **磁盘垃圾清理**(`cpp/service/src/JunkCleaner.cpp` + 「垃圾清理」页面)
  - 23 类。系统侧:临时文件、回收站、Windows 更新缓存、传递优化缓存、错误报告与内存转储、缩略图 / 图标缓存、预读取文件、字体缓存、系统安装与维护日志、Installer 补丁缓存、Defender 扫描历史、系统升级与重置残留、网络临时文件。应用侧:浏览器缓存、显卡着色器缓存、开发包管理器缓存(pip / npm / NuGet / Gradle / Maven / Cargo / Go)、游戏平台缓存、Office 文档缓存、最近使用记录。本产品自身:**可重建缓存**(信誉缓存 / 攻击链组合表 / 诊断日志 / 更新暂存)与**过期日志**分成两类 —— 前者删了只是多一次网络往返(Safe、默认勾选),后者会丢事后追查的证据(Caution、默认不勾),混成一类会迫使用户在「想清缓存」和「不想丢证据」之间做一个本不该存在的取舍
  - 本产品自身目录下**刻意不纳入**的每一项都在代码里写明了理由:规则库与设置(用户策略)、隔离区(可还原的样本)、事件历史(取证)、`seen_hashes.txt`(删了所有程序重新变成「首见」,一批软信号同时点亮 → 误报上升)、`baseline.json`(重新进入学习期)、`tb_ip_quota.json`(额度记账清零会导致超配额被限流,删它是**有害**的)、待上传的情报贡献、机器标识、云信誉查询历史(该有自己的「清空」入口)
  - 「系统保留空间」(休眠文件 13 GB + 页面文件 8.7 GB)走独立的**体积探针**路径:只 stat、不进遍历更不进删除代码,且不计入「可释放空间」总量 —— 把它算进去等于虚报
  - **扫描结果的可信度是可核对的**,这三点都是「让用户能判断这个数字能不能信」:
    - **读不进去的目录如实计数**。`QDir::entryInfoList` 对「无权限」与「空目录」返回同样的空列表,原实现会把「看不进去」静默算成「这里没东西」—— 用户看到一个偏小的数字且毫无提示。现在用 Win32 `FindFirstFileW` 把两者分开(`ERROR_ACCESS_DENIED` 即不可读),逐位置计数并在界面上标黄。实测本机 `INetCache` 下确有 2 个受保护子目录命中这条路径
    - **耗时与检视文件数写进结论**。扫描只读目录元数据(不打开文件、不算哈希),上万个条目一秒出头是正常的,但「一下就完了」很容易被当成没扫 —— 报出「耗时 1.77 秒,检视 11466 个文件」之后它就变成可核对的数字而不是可疑现象。逐类别也各有耗时
    - **按扩展名过滤的位置自动说明范围**。实测 `C:\Windows\Logs` 下 269 个过期文件里只有 246 个在清理范围内(刻意不动 `.evtx`),这个差额原本完全不可见;现在位置行直接写「仅清理 *.log / *.etl / *.dpx / *.cab」
  - 扫描与清理都带**位置级实时进度**(200ms 节流)。原先只在切换类别时报一次,一个位置扫十几秒时界面完全不动,与卡死无法区分
  - 扫描 → 树形展示各类体积与**逐个位置的明细** → 勾选 → 二次确认 → 清理并逐类汇报。扫描与清理都在后台线程,经 `JunkProgressNotification` 报进度(纳入停机等待,与取证查询同一套路)
  - **IPC 请求只携带类别序号,不携带任何路径**。每个类别对应一组编译期写死的根目录 —— 一个接受调用方给定路径的删除接口就是任意文件删除原语,哪怕管道那头已强制校验客户端映像也不该存在
  - 七道护栏:根目录准入校验、规范化后的包含性校验(防 `%TEMP%` 里的 junction 把遍历带到 System32)、不跟随重解析点、保留时长阈值(默认 24h,避免弄坏正在进行的安装)、占用即跳过**绝不升级手段**、条数 / 深度 / 时间上限、「绝不触碰」名单(安装目录、隔离区金库、System32 / WinSxS / Servicing、文档 / 桌面 / 图片 / 视频、用户信任名单)
  - 浏览器缓存只进 `Cache` / `Code Cache` / `GPUCache` / `Service Worker\{CacheStorage,ScriptCache}`,**绝不碰配置目录本身** —— Cookie、历史、密码、书签不是垃圾
  - 高风险类别(回收站 / 预读取 / 最近使用记录 / 本产品日志)默认不勾选,确认框里单独列出副作用
  - 两条有意的产品决定:回收站走 `SHQueryRecycleBin` / `SHEmptyRecycleBin` 整体清空(不手工遍历 `$Recycle.Bin`、不按保留时长挑选);「旧版 Windows 升级残留」**只报告体积、不代为删除**(正确删除要逐个接管上万个受 ACL 保护文件的所有权,删一半比不删更糟,且删完再无法回退系统升级)
  - `--junk-scan`:只读干跑诊断入口,不起事件循环 / ETW / 驱动 / IPC,用于验证「它认为自己该动哪些位置」
  - 配置段 `DiskCleanup`(开关 / 保留时长 / 上限 / 额外排除)。**清理范围不在配置里**,`ExcludePaths` 只能让范围变小
- **大文件查找**(`LargeFileScanner`,与按类别清理同页两个标签)
  - 占地方的往往不是垃圾,而是几个自己都忘了的大文件。实测本机 557486 个文件 / 44 秒,列出最大的 30 个共 198 GB:D 盘回收站里一个 14 GB 的 zip、11 GB 的模拟器磁盘、10 GB 的虚拟机磁盘、两个 7.9 GB 的 ISO —— 这些不属于任何「垃圾」类别,但确实是用户想知道的
  - **纯只读,而且刻意不提供删除**。它报的是任意路径上的任意文件,一旦配一个删除接口,整条链路就变成任意文件删除原语,与按类别清理那边「IPC 只收类别序号、绝不收路径」的护栏直接对立。界面只给「在资源管理器中打开所在位置」,由用户自己处置 —— 这不是功能没做完,是刻意的边界。为此 IPC 只有 `LargeFileScanRequest/Response` 一对,没有第三条消息
  - 范围固定为本机固定磁盘(排除网络盘与可移动介质:遍历网络共享要几分钟且吃满带宽,U 盘插拔后结果立刻过期)。跳过系统组件目录与安装目录 —— 把 WinSxS 里几百个组件列进「你可以清理的大文件」只会淹没真正有用的那几行
  - 用有序插入维持 Top-N 而不是「全收完再排序」:阈值给低时全盘可能有上百万个文件过线,全收进内存等于点一下就吃掉几百 MB;收满后门槛自动抬高,后面更小的连插入都不做
  - `--large-files [阈值MB]`:只读干跑诊断入口(与 `--junk-scan` 同一模式)
